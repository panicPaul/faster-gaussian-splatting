    ...
    const bool proper_antialiasing)
{
    ...
    if (config::original_opacity_interpretation && opacity < config::min_alpha_threshold)
        active = false;
    ...
    if (determinant < config::min_cov2d_determinant)
        active = false;
    ...
    const uint grid_width)
{
    ...
    for (int n_points_remaining = n_points_total, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
        ...
        for (int j = 0; !done && j < current_batch_size; ++j) {
            ...
            if (!config::original_opacity_interpretation && gaussian < config::min_alpha_threshold)
                continue;
            const float alpha = opacity * gaussian;
            if (config::original_opacity_interpretation && alpha < config::min_alpha_threshold)
                continue;
    ...
    n_processed_and_used = BlockReduce(temp_storage).Reduce(n_processed_and_used, cuda::std::maximum<uint>());
    if (thread_rank == 0)
